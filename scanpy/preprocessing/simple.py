...
def filter_cells(data, min_counts=None, min_genes=None, max_counts=None,
    ...
    if isinstance(data, AnnData):
        ...
        cell_subset, number = compute_if_dask_array(filter_cells(adata.X, min_counts, min_genes, max_counts, max_genes))
...
def filter_genes(data, min_counts=None, min_cells=None, max_counts=None,
    ...
    if isinstance(data, AnnData):
        adata = data.copy() if copy else data
        gene_subset, number = compute_if_dask_array(filter_genes(adata.X, min_cells=min_cells,
...
def filter_genes_dispersion(data,
    ...
    mean, var = compute_if_dask_array(_get_mean_var(X))
...
def normalize_per_cell(data, counts_per_cell_after=None, counts_per_cell=None,
    ...
    if isinstance(data, AnnData):
        ...
        cell_subset_np, counts_per_cell_np = compute_if_dask_array([cell_subset, counts_per_cell])
    ...
    if not issparse(X): X /= compute_if_dask_array0(counts_per_cell[:, np.newaxis])
